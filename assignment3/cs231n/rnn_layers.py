...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for i in range(T):
        hprev = h0 if i == 0 else h[:, i-1, :]
        h_i, cache_i = rnn_step_forward(x[:, i, :], hprev, Wx, Wh, b)
        h[:, i, :] = h_i
...
def rnn_backward(dh, cache):
    ...
    # .   next_h, cache = (x, prev_h, Wx, Wh, b, next_h)
    #
    #
    # rnn_step_backward(dnext_h, cache) -> dx, dprev_h, dWx, dWh, db

    # x, h0, Wx, Wh, out = cache

    (x, prev_h, Wx, Wh, b, next_h) = cache[-1]
    ...
    for i in reversed(range(T)):
        dxi, dhprev, dWxi, dWhi, dbi = rnn_step_backward(
            dh[:, i, :]+dhprev, cache[i])
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = x, Wx, Wh, prev_h, prev_c, i, f, o, g, tanh_c
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, Wx, Wh, prev_h, prev_c, i, f, o, g, tanh_c = cache

    dtanh_c = o * (1 - tanh_c ** 2) * dnext_h
    dprev_c = f * (dtanh_c + dnext_c)
    df = f * (1 - f) * prev_c * (dnext_c + dtanh_c)
    do = o * (1 - o) * dnext_h * tanh_c
    dg = (1 - g**2) * i * (dnext_c + dtanh_c)
    di = i * (1 - i) * g * (dnext_c + dtanh_c)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        ...
        h_i, c, cache_i = lstm_step_forward(x[:, i, :], prev_h, c, Wx, Wh, b)
        h[:, i, :] = h_i
        cache.append(cache_i)
...
def lstm_backward(dh, cache):
    ...
    x, Wx, Wh, prev_h, prev_c, i, f, o, g, tanh_c = cache[-1]
    ...
    for i in reversed(range(T)):
        dxi, dprev_h, dprev_c, dWxi, dWhi, dbi = lstm_step_backward(
